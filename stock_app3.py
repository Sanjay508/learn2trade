...
if not st.session_state.authenticated:
    ...
    if method_trading == "Learn Concepts" or st.session_state.show_learning:
        ...
        if st.session_state.selected_course and st.session_state.selected_lesson:
            ...
            with col1:
                if st.button("📖 Read More", use_container_width=True, key="read_more"):
                    st.info("Check out 'The Intelligent Investor' by Benjamin Graham")
            with col2:
                if st.button("🎥 Watch Video", use_container_width=True, key="watch_video"):
                    st.info("Search for 'Stock Market Basics' on YouTube")
            with col3:
                if st.button("💬 Discuss", use_container_width=True, key="discuss"):
                    ...
                    try:
                        if selected_period == "Live":
                            ...
                            if company.endswith('.NS') and not market_open:
                                st.warning("⚠️ Indian Market is currently closed. Showing last available data.")
                            ...
                            with trading_col1:
                                ...
                                if company.endswith('.NS') and not is_market_open:
                                    ...
                                    if max_shares > 0:
                                        ...
                                        st.warning("⚠️ Insufficient cash to buy shares")
        ...
        with change_trade_cols[0]:
            if st.button("🔄 Start New Trade", use_container_width=True, type="primary"):
                ...
                if shares_held > 0:
                    st.warning("⚠️ You currently hold shares!")
                    option = st.radio(
